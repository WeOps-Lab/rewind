...
class InstallerViewSet(ViewSet):
            ...
            properties={
                ...
                'nodes': openapi.Schema(
                    ...
                    properties={
                        ...
                        'organizations': openapi.Schema(type=openapi.TYPE_ARRAY,items=openapi.TYPE_STRING,description="所属组织"),
                    }
                ),
            }
        ),
    ...
    def controller_install(self, request):
        task_id = InstallerService.install_controller(request.data["cloud_region_id"], request.data["work_node"], request.data["nodes"])
    ...
    def controller_uninstall(self, request):
        uninstall_controller.delay(request.data)
    ...
    # def sidecar_uninstall(self, request):
    #     pass
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            items=openapi.Schema(type=openapi.TYPE_STRING,description="节点ID")
        ),
        tags=['Installer']
    )
    @action(detail=False, methods=["post"], url_path="collector/install")
    def collector_install(self, request):
        task_id = InstallerService.install_collector(request.data)
    ...
    # def collector_uninstall(self, request):
    #     pass
