...
class LLMSkillAdmin(ModelAdmin):
    ...
    ordering = ["id"]
    filter_horizontal = ["knowledge_base"]
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        只允许选择自己的知识库
        """
        if db_field.name == "knowledge_base":
            kwargs["queryset"] = KnowledgeBase.objects.filter(created_by=request.user.username)
    ...
    fieldsets = [
        ...
        (
            "知识库",
            {
                "fields": ["enable_rag", "knowledge_base", "enable_rag_knowledge_source", "rag_score_threshold"],
            },
        ),
    ]
